...
/**
 * Created by wlo on 3/10/2016.
 * This is for Junit test for greeting controller
 */
...
public class GreetingControllerTest {
    ...
    public void greetingAuthorized() throws Exception {
        ...
        mvc.perform(get("/greeting")
                ...
                .andExpect(jsonPath("$.content", is("Hello, Roy!")));
        // @formatter:on
        System.out.println("/monitorkeyword/index");
        MvcResult result = mvc.perform(
                get("/monitorkeyword/index")
                //.contentType(MediaType.APPLICATION_FORM_URLENCODED)
                .header("Authorization", "Bearer " + accessToken)
                .param("username", "testbd"))
                .andExpect(status().isOk())
                .andReturn() ;
                //.andExpect(jsonPath("$.id", is(3)))
                //.andExpect(jsonPath("$.content", is("Hello, Roy!")));
        String content = result.getResponse().getContentAsString();
        System.out.println("monitorkeyword index result:"+content);
    }

    @Test
    public void testNewSavekeyword() throws Exception {
        String accessToken = getAccessToken("wlo", "Password123");
        /*MvcResult result =
                mvc.perform(get("/monitorKeyword/saveUserKeyword?userid=1%26word=msp")
                        .header("Authorization", "Bearer " + accessToken))
                        .andExpect(status().isOk())
                        //.andExpect(jsonPath("$", hasSize(3)))
                        .andReturn();

        System.out.println("save keyword result:"+ result.toString());*/
